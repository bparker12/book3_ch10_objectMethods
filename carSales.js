...
function dataReturn(name) {
    return `
<h3>${name.sales_agent.first_name} ${name.sales_agent.last_name}</h3>
`
}
function profitReturn(profit) {
    return `
    <li>Profit: $${profit.gross_profit}</li>
    `
}

function postToDom(name) {
    weeklySales.innerHTML += dataReturn(name)
    dataReturn(name)
}

// let vehicleInfo = salesByWeek.

salesByWeek.forEach(agents => {
    weeklySales.innerHTML += dataReturn(agents)
    // for (let agent of Object.values(agents.sales_agent))
    // weeklySales.innerHTML += `<h3>${agent[0]}</h3>`
    console.log(agents)
    for (let sale of Object.entries(agents["vehicle"])) {
        weeklySales.innerHTML += `<li>${sale[0]}: ${sale[1]}</li>`
    }
    // for (let profit of Object.values(salesByWeek)) {
    //     console.log(profit.gross_profit)
    console.log(agents.gross_profit)
        weeklySales.innerHTML += profitReturn(agents)
